// @ https://github.com/autobotoptimusprime/cc_hashlib/blob/main/README.md
#pragma once
#include <algorithm>
#include <array>
#include <cstdint>
...
public:
  ...
  auto digest() -> std::array<uint32_t, 4> {
    ...
    std::array<uint32_t, 4> magic_nums{0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476}, abcd{};
    ...
    static std::array<size_t, 16> S{7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21};
  ...
  auto update(const std::string &str) -> void { std::copy(str.begin(), str.end(), std::back_inserter(__seq)); }
  ...
  auto clear() -> void { __seq.clear(); }
  auto empty() -> bool { return __seq.empty(); }
